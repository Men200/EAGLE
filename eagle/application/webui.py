...
def warmup(model):
    ...
    for output_ids in model.ea_generate(input_ids): # ea_generate采用yield而非return，意味着它是一个python生成器（generator）
        # 每次调用会返回部分结果并暂停执行，等待下次请求继续生成
...
parser.add_argument("--base-model-path", type=str, default="/home/xwwen/EAGLE_test/Llama-2-7b-chat-hf/",
